...
export default function SellForm() {
  ...
  // Default token for selling
  useEffect(() => {
    if (!currentToken && tokens.length > 0) {
      const defaultToken = tokens.find((token) => token.symbol === 'ETH');
      if (defaultToken) {
  ...
  async function executeSell() {
    ...
    try {
      ...
      toast.success('Sell completed successfully!');
    ...
    <>
      <form>
        ...
        <div className='mb-6'>
          <div className='bg-gray-50 border border-gray-200 rounded-[12px] p-4'>
            <div className='flex items-center justify-between mb-2'>
              <span className='text-sm text-gray-600'>You Receive (USD)</span>
            </div>
